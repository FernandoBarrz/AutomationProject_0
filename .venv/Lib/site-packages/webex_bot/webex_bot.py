...
class WebexBot(WebexWebsocketClient):
    ...
    def __init__(self,
        ...
        self.add_command(CSVCommand(self.request))
    ...
    def process_raw_command(self, raw_message, teams_message, user_email, activity, is_card_callback_command=False):
        ...
        log.warning(teams_message)
        log.warning(self.files)

        log.warning(str(teams_message))
        new_teams_message = str(teams_message).replace("Webex Teams Message:","")
        log.warning(str(new_teams_message))

        if self.files != None:
            parseData = json.loads(str(new_teams_message))
            

            attach = parseData['files'][0]

            request = requests.get(attach,headers={"Authorization": f"Bearer {self.teams_bot_token}"})
            ...
            log.warning(attach)
            command = CSVCommand(request)
            return

ommand}")
t self.check_user_approved(user_email=user_email, approved_rooms=command.approved_rooms):
